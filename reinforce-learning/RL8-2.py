...
def check_model_nan(model, name="model"):
    for n, p in model.named_parameters():
        if torch.isnan(p).any() or torch.isinf(p).any():
            print(f"[Debug] {name} param '{n}' contains NaN or Inf!")
            return True
    return False
...
class DQNAgent(nn.Module):
    ...
    def replay(self, batch_size):
        if check_model_nan(self.plan_model, "plan_model"):
            print("[Replay] plan_model is polluted, skipping the training")
            return
        ...
        for state, plan_action, task_action, reward, next_state, done in minibatch:
            ...
            state_tensor = torch.FloatTensor(state) / 20000.0
            next_state_tensor = torch.FloatTensor(next_state) / 20000.0

            with torch.no_grad():
                target_q = self.target_model(next_state_tensor)

                max_next_q = torch.max(target_q).item()
                target_value = reward + self.gamma * max_next_q if not done else reward


            current_q = self.plan_model(state_tensor)
            # 防越界处理
            if plan_action >= current_q.shape[0]:
                continue

            target_f = current_q.clone().detach()
            target_f[plan_action] = target_value
            loss = self.criterion(current_q, target_f)

            if torch.isnan(loss) or torch.isinf(loss):
                print("[Replay] Loss is NaN/Inf. Skip update.")
                continue

            self.optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(self.plan_model.parameters(), 1.0)
            self.optimizer.step()
    ...
    def update_target_model(self):
        """软更新目标网络"""
        if not check_model_nan(self.plan_model):
            self.target_model.load_state_dict(self.plan_model.state_dict())
        else:
            print("[Update] skipping updating, plan_model is polluted.")
