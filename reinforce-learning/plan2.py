...
def compare_and_output():
    ...
    # 运行 non-overlap.py
    non_overlap_mips, non_overlap_remaining_mem, non_overlap_latency = run_and_monitor('non-overlap.py')

    # 计算最大值
    max_mips = max(overlap_mips, non_overlap_mips) if overlap_mips and non_overlap_mips else None
    max_remaining_mem = max(overlap_remaining_mem, non_overlap_remaining_mem, key=lambda x: float(x.split()[0]))
    max_latency = max(overlap_latency, non_overlap_latency)
